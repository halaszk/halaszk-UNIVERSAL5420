...
static int max77803_read_byte(struct i2c_client *client, u8 reg, u8 *intval)
{
	int ret, i;

	ret = max77803_read_reg(client, reg, intval);
	if (unlikely(ret < 0)) {
		pr_err("%s: max77803 read byte error,%d\n", __func__, ret);
		for (i = 0; i < 5; i++) {
			ret = max77803_read_reg(client, reg, intval);
			if (ret >= 0) {
				pr_info("%s: changed reg : 0x%x, value : 0x%x\n",
					__func__, reg, *intval);
				break;
			}
			pr_err("%s: err reg : 0x%x, value : 0x%x\n", __func__, reg, *intval);
		}
	}
	pr_debug("%s: read value : 0x%x\n", __func__, *intval);

	return ret;
}

static int max77803_write_byte(struct i2c_client *client, u8 reg, u8 intval)
{
	int ret, i;

	ret = max77803_write_reg(client, reg, intval);
	if (unlikely(ret < 0)) {
		pr_err("%s: max77803 write byte error %d\n", __func__, ret);
		for (i = 0; i < 5; i++) {
			ret = max77803_write_reg(client, reg, intval);
			if (ret >= 0) {
				pr_info("%s: changed reg : 0x%x, value : 0x%x\n",
...
static void max77803_dump_reg(struct max77803_charger_data *charger)
{
	...
	for (reg_addr = 0xB0; reg_addr <= 0xC5; reg_addr++) {
		max77803_read_byte(charger->max77803->i2c, reg_addr, &reg_data);
...
{
	u8 reg_data, bat_data;

	if (max77803_read_byte(charger->max77803->i2c,
	...
	pr_info("%s: CHG_INT_OK(0x%02x)\n", __func__, reg_data);

	reg_data = ((reg_data & MAX77803_BATP_OK) >> MAX77803_BATP_OK_SHIFT);

	max77803_read_byte(charger->max77803->i2c,
			MAX77803_CHG_REG_CHG_DTLS_00, &bat_data);
	bat_data = ((bat_data & MAX77803_BATP_DTLS) >> MAX77803_BATP_DTLS_SHIFT);

	if ((bat_data == 0x01) && (reg_data == 0x00))
		reg_data = 0x00;
	else
		reg_data = 0x01;
...
static int max77803_get_health_state(struct max77803_charger_data *charger)
{
	...
	if ((state == POWER_SUPPLY_HEALTH_UNSPEC_FAILURE) && \
		(max77803_get_battery_present(charger)))
		state = POWER_SUPPLY_HEALTH_GOOD;
...
static bool max77803_charger_unlock(struct max77803_charger_data *chg_data)
{
	...
	max77803_read_byte(i2c, MAX77803_CHG_REG_CHG_CNFG_06, &reg_data);
	...
	do {
		max77803_write_byte(i2c, MAX77803_CHG_REG_CHG_CNFG_06,
					(0x03 << 2));
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (val->intval == POWER_SUPPLY_TYPE_BATTERY) {
			...
			if (charger->cable_type == POWER_SUPPLY_TYPE_UARTOFF) {
				u8 reg_data;
				max77803_read_byte(charger->max77803->muic,
					MAX77803_MUIC_REG_CDETCTRL2, &reg_data);
				reg_data |= (0x01 << 0);
				max77803_write_byte(charger->max77803->muic,
					MAX77803_MUIC_REG_CDETCTRL2, reg_data);
...
static void wpc_detect_work(struct work_struct *work)
{
	...
	max77803_read_byte(chg_data->max77803->i2c,
...
static irqreturn_t max77803_bypass_irq(int irq, void *data)
{
	...
	if (byp_dtls & 0x1) {
		...
		chg_cnfg_00 &= ~(CHG_CNFG_00_OTG_MASK
				| CHG_CNFG_00_BOOST_MASK
				| CHG_CNFG_00_DIS_MUIC_CTRL_MASK);
		max77803_write_byte(chg_data->max77803->i2c,
...
static __devinit int max77803_charger_probe(struct platform_device *pdev)
{
	...
	if (max77803_read_byte(charger->max77803->i2c, MAX77803_PMIC_REG_PMIC_ID1, &reg_data) < 0) {
