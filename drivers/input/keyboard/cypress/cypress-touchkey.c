...
void touchkey_grip_mode_ctr(struct touchkey_i2c *tkey_i2c)
{
	...
	printk(KERN_DEBUG"touchkey:grip mode %s\n",
		(tkey_i2c->ic_mode == MODE_GRIP) ? "on" : "off");
...
#ifdef TKEY_GRIP_MODE
	if (tkey_i2c->ic_mode == MODE_GRIP)
		printk(KERN_DEBUG"touchkey:grip mode off\n");
	tkey_i2c->ic_mode = MODE_NORMAL;
	tkey_i2c->pdata->power_on(0);
	msleep(20);
#endif
