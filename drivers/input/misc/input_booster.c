...
DECLARE_DVFS_DELAYED_WORK_FUNC(CHG, TOUCHKEY)
{
	...
	if (dvfs->level == BOOSTER_LEVEL1) {
		...
		set_qos(&dvfs->cpu_qos, PM_QOS_KFC_FREQ_MIN, dvfs->freqs[BOOSTER_LEVEL2].cpu_freq);
...
DECLARE_DVFS_DELAYED_WORK_FUNC(CHG, TOUCH)
{
	...
	break;
	case BOOSTER_LEVEL4:
		set_qos(&dvfs->cpu_qos, PM_QOS_KFC_FREQ_MIN, dvfs->freqs[BOOSTER_LEVEL4].cpu_freq);
