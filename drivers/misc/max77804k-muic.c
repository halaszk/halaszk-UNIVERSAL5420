					...
					u8 status1, u8 status2)
{
	...
	case ADC_GND:
		if (status1 == 0x20) {
			pr_info("%s:%s ADC_GND & !status1 = OTG\n", DEV_NAME,
					__func__);
			break;
		}
		pr_info("%s:%s ADC_GND & status1 != OTG\n", DEV_NAME,
...
static void max77803_muic_detect_dev(struct max77803_muic_info *info, int irq)
{
	...
	ret = max77803_read_reg(client, MAX77803_MUIC_REG_CTRL4, &cntl4_val);
    dev_info(info->dev, "func:%s CONTROL4:%x\n", __func__, cntl4_val);
    if ((cntl1_val != 0x1b) && (cntl4_val == 0xf2))
	{
		cntl4_val = 0xb2;
        ret = max77803_write_reg(client, MAX77803_MUIC_REG_CTRL4, cntl4_val);
		ret = max77803_read_reg(client, MAX77803_MUIC_REG_CTRL4, &cntl4_val);
		pr_info("%s: Change ADCMODE: %x\n", __func__, cntl4_val);
		mdelay(50);
	} // Not JIG, ADCMODE_Pulse -> Change ADCMODE_One shot
