...
static int get_anchored_cnt(struct usb_anchor *urbs)
{
	int cnt = 0;
	struct list_head *pos;

	list_for_each (pos, &urbs->urb_list) {
		if (cnt++ > MULTI_URB - 1)
			panic("[HSIC] Abnormal urb_list with(%d)\n", cnt);
...
static void if_usb_debug_work(struct work_struct *work)
{
	struct usb_link_device *usb_ld =
		container_of(work, struct usb_link_device, link_debug.work);
	...
	if (!usb_ld->if_usb_connected) {
		mif_info("link not connected\n");
		return;
	}

	rpm_status = usb_get_rpm_status(&udev->dev);
	if (rpm_status == RPM_SUSPENDING) {
		for (cnt = 0; cnt < usb_ld->max_link_ch; cnt++) {
			pipe_data = get_pipedata_with_idx(usb_ld, cnt);
			copied += sprintf(tmp + copied, "[%d:%d,%d] ", cnt,
					get_anchored_cnt(&pipe_data->reading),
					get_anchored_cnt(&pipe_data->urbs));
		}
		mif_info("[rpm=%d] %s [suspended=%d]\n",
				rpm_status, tmp, usb_ld->suspended);
		if (usb_ld->debug_cnt++ > 10)
			panic("[HSIC] Abnormal rpm_status\n");
		else
			queue_delayed_work(usb_ld->wq, &usb_ld->link_debug,
...
static int if_usb_suspend(struct usb_interface *intf, pm_message_t message)
{
	...
	if (!usb_ld->suspended) {
		mif_info("Start to enter suspend intf\n");
		queue_delayed_work(usb_ld->wq, &usb_ld->link_debug,
	...
	if (usb_ld->suspended == usb_ld->max_link_ch * 2) {
		mif_info("[if_usb_suspended] tx_cnt=%d, rx_cnt=%d\n",
			usb_ld->tx_cnt, usb_ld->rx_cnt);
...
static int if_usb_resume(struct usb_interface *intf)
{
	...
	if (!pipe_data->usb_ld->suspended) {
		pipe_data->usb_ld->debug_cnt = 0;
		cancel_delayed_work_sync(&pipe_data->usb_ld->link_debug);
		mif_info("[if_usb_resumed] tx_cnt=%d, rx_cnt=%d\n",
			pipe_data->usb_ld->tx_cnt, pipe_data->usb_ld->rx_cnt);
		mif_com_log(pipe_data->iod->msd, "Called %s func\n", __func__);
	}
	return 0;
...
static int if_usb_init(struct link_device *ld)
{
	...
	INIT_DELAYED_WORK(&usb_ld->link_debug, if_usb_debug_work);
	usb_ld->wq = create_singlethread_workqueue("link_debug");
	if (!usb_ld->wq) {
		mif_err("failed to create link_debug wq\n");
		return -EINVAL;
	}
