...
static int link_pm_usb_hdev_check_resume(struct link_pm_data *pmdata)
{
	...
	if (pmdata->hdev && pmdata->hdev->state != USB_STATE_SUSPENDED) {
		spin_unlock_irqrestore(&pmdata->lock, flags);
		return 0;
	}
	spin_unlock_irqrestore(&pmdata->lock, flags);

	mif_info("%s resume first\n", dev_name(&pmdata->hdev->dev));

	ret = pm_runtime_resume(&pmdata->hdev->dev);
	if (ret < 0) {
		mif_err("%s pm_runtime_resume fail(%d)\n",
			dev_name(&pmdata->hdev->dev), ret);
		return ret;
	}
...
static int link_pm_usb_resume(struct usb_device *udev, pm_message_t msg)
{
	...
	case LINK_PM_L3:
		if (!PMSG_IS_AUTO(msg) &&
				!gpio_get_value(pdata->gpio_link_hostwake)) {
			mif_info("return dpm_resume\n");
			if (wake_lock_active(&pmdata->wake))
				wake_lock_timeout(&pmdata->wake,
						msecs_to_jiffies(50));
			udev->can_submit = 0;
			return -EBUSY;
		}
...
generic_resume:
	...
	if (pdata && pmdata) {
		if (udev == pmdata->hdev) {
			...
			mif_info("%s resumed by pm event %x hostactive = %d\n",
					dev_name(&udev->dev), msg.event,
					gpio_get_value(pdata->gpio_link_active));

		} else {
			mif_info("%s resumed - %d/%d/%d\n",dev_name(&udev->dev),
				gpio_get_value(pdata->gpio_link_slavewake),
				gpio_get_value(pdata->gpio_link_hostwake),
				gpio_get_value(pdata->gpio_link_active));
		}
...
static int link_pm_pm_qos_notify(struct notifier_block *nfb,
						unsigned long event, void *arg)
{
	struct link_pm_data *pmdata =
		container_of(nfb, struct link_pm_data, pm_qos_nfb);
...
static int link_pm_probe(struct platform_device *pdev)
{
	...
	pmdata->pm_qos_nfb.notifier_call = link_pm_pm_qos_notify;
