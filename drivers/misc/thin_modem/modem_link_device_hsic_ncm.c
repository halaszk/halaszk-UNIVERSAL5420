		...
		struct urb *urb, gfp_t gfp_flags)
{
	...
	ret = usb_submit_urb(urb, gfp_flags | __GFP_NOWARN);
	if (ret) {
...
static int if_usb_suspend(struct usb_interface *intf, pm_message_t message)
{
	...
	if (pmdata && pmdata->wait_cp_suspend_ready)
		pmdata->wait_cp_suspend_ready(pmdata, pipe_data->usbdev);
#endif
...
static int xmm626x_phy_notify(struct notifier_block *nfb,
...
static int if_usb_init(struct link_device *ld)
{
	...
	usb_ld->phy_nfb.notifier_call = xmm626x_phy_notify;
