...
static int __init get_kfc_freq(char *str)
{
        get_option(&str, &set_kfc_freq);
        return 0;
}
early_param("kfcfreq", get_kfc_freq);
#endif
...
#ifdef CONFIG_ASV_MARGIN_TEST
        if (cur == CA15 && set_cpu_freq > 0) {
                target_freq = set_cpu_freq;
        } else if (cur == CA7 && set_kfc_freq > 0) {
                target_freq = set_kfc_freq;
        }
#endif
