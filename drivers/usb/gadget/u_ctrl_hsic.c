...
static int ghsic_ctrl_receive(void *dev, void *buf, size_t actual)
{
	...
	pr_debug("[USB][HSIC] %s: read complete bytes read: %d\n",
			__func__, actual);
...
ghsic_send_cpkt_tomodem(u8 portno, void *buf, size_t len)
{
	...
	pr_debug("[USB][HSIC] %s: ctrl_pkt:%d bytes\n", __func__, len);
...
static void ghsic_ctrl_connect_w(struct work_struct *w)
{
	...
	pr_debug("[BIT][USB][HSIC] %s: port:%p\n", __func__, port);
	...
	pr_debug("[BIT][USB][HSIC] %s : set bit - CH_OPENED(sts=%ld)\n", __func__, port->bridge_sts);
	...
	pr_debug("[BIT][USB][HSIC] %s : result (sts=%ld)\n", __func__, port->bridge_sts);
...
static void gctrl_disconnect_w(struct work_struct *w)
{
	...
	pr_debug("[BIT][USB][HSIC] %s\n", __func__);

	if (!test_bit(CH_OPENED, &port->bridge_sts))
		return;

	/* send the dtr zero */
	pr_debug("[BIT][USB][HSIC] %s Clear CH_OPENED\n", __func__);
	ctrl_bridge_close(port->brdg.ch_id);
...
static int ghsic_ctrl_probe(struct platform_device *pdev)
{
	...
	pr_debug("[BIT][USB][HSIC] %s: name:%s\n", __func__, pdev->name);
	...
	pr_debug("[BIT][USB][HSIC] %s :  set bit - CH_READY (sts=%ld)\n", __func__, port->bridge_sts);
...
static int ghsic_ctrl_remove(struct platform_device *pdev)
{
	...
	clear_bit(CH_OPENED, &port->bridge_sts);
not_ready:
	pr_debug("[BIT][USB][HSIC] %s Clear CH_READY \n", __func__);
...
static int gctrl_port_alloc(int portno, enum gadget_type gtype)
{
	...
	pr_debug("[USB][HSIC] %s: port:%p portno:%d\n", __func__, port, portno);
...
int ghsic_ctrl_setup(unsigned int num_ports, enum gadget_type gtype)
{
	...
	pr_debug("[USB][HSIC] %s: requested ports:%d\n", __func__, num_ports);
