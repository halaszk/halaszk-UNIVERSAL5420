...
static void ghsic_data_start_io(struct gdata_port *port)
{
	...
	if (ret) {
		pr_info("[USB][HSIC] %s: rx req allocation failed\n", __func__);
		spin_unlock_irqrestore(&port->rx_lock, flags);
		return;
	}
	spin_unlock_irqrestore(&port->rx_lock, flags);
	...
	if (ret) {
		pr_info("[USB][HSIC] %s: tx req allocation failed\n", __func__);
...
static void ghsic_data_connect_w(struct work_struct *w)
{
	...
	pr_info("[BIT][USB][HSIC] %s\n", __func__);

	if (!port || !atomic_read(&port->connected) ||
		!test_bit(CH_READY, &port->bridge_sts))
		return;

	pr_info("[BIT][USB][HSIC] %s: port:%p\n", __func__, port);
	...
	pr_info("[BIT][USB][HSIC] %s : set bit - CH_OPENED (sts=%ld)\n", __func__, port->bridge_sts);
	set_bit(CH_OPENED, &port->bridge_sts);
	pr_info("[BIT][USB][HSIC] %s : result (sts=%ld)\n", __func__, port->bridge_sts);
...
static void ghsic_data_disconnect_w(struct work_struct *w)
{
	...
	pr_info("[BIT][USB][HSIC] %s Clear CH_OPENED \n", __func__);
...
static int ghsic_data_probe(struct platform_device *pdev)
{
	...
	pr_debug("[BIT][USB][HSIC] %s: name:%s no_data_ports= %d\n",
		__func__, pdev->name, no_data_ports);

	if (pdev->id >= no_data_ports) {
		pr_debug("[USB][HSIC] %s: invalid port: %d\n", __func__, pdev->id);
		return -EINVAL;
	}

	port = gdata_ports[pdev->id].port;
	pr_debug("[BIT][USB][HSIC] %s : set bit - CH_READY\n", __func__);
...
static int ghsic_data_remove(struct platform_device *pdev)
{
	...
	pr_debug("[BIT][USB][HSIC] %s Clear CH_OPENED, CH_READY \n", __func__);
...
//		out_desc = gser->out_desc;
	...
	if (ret) {
		pr_info("[USB][HSIC] %s: usb_ep_enable failed eptype:OUT ep:%p",
				__func__, port->out);
		usb_ep_disable(port->in);
		goto fail;
	}
