...
static int gport_setup(struct usb_configuration *c)
{
	...
	pr_debug("[USB][HSIC] %s: no_hsic_sports: %u nr_ports: %u\n",
	...
	if (no_hsic_sports) {
		...
		if (port_idx < 0)
			{
				pr_debug("usb: [JDJ] %s: ghsic_data_setup error : \n",
			__func__);
			return port_idx;
			}
		...
		if (ret < 0)
		{
			pr_debug("usb:[JDJ] gport_setup error return [%d] \r\n",ret);
			return ret;
		}
		return 0;
	}
...
static int gserial_init_port(int port_num, const char *name)
{
	...
	{
		pr_debug("usb:[JDJ] gserial_init_port - No port \r\n");
		return -ENODEV;
	}
	...
	nr_ports++;
	pr_debug("usb:[JDJ] gserial_init_port return : nr_ports=%d / no_hsic_sports=%d \r\n",nr_ports,no_hsic_sports);
	return 0;
}
