...
static unsigned char ak7343_default_pid;
...
int fimc_is_sec_ak7343_get_ack(struct fimc_is_i2c_gpio *ak7343)
{
	int i;
	int ret = -EIO;
	gpio_set_value(ak7343->scl->pin, 1);

	for (i = 0; i < 10; i++)
	{
		if (!gpio_get_value(ak7343->sda->pin)) {
			ret = 0;
			break;
		}
		usleep_range(1, 1);
	}
	gpio_set_value(ak7343->scl->pin, 0);
	usleep_range(1, 1);
	return ret;
}
...
int fimc_is_sec_ak7343_output(struct fimc_is_i2c_gpio *ak7343, unsigned char data)
{
	...
	for (i = 0; i < 8; i++) {
		...
		if (data & (0x80 >> i))
			...
			sda_value = 0;
		gpio_set_value(ak7343->sda->pin, sda_value);
		usleep_range(1, 1);
		gpio_set_value(ak7343->scl->pin, 1);
		usleep_range(2, 2);
		gpio_set_value(ak7343->scl->pin, 0);
		usleep_range(1, 1);
		gpio_set_value(ak7343->sda->pin, 0);
		usleep_range(1, 1);
	}
	s3c_gpio_cfgpin(ak7343->sda->pin, S3C_GPIO_INPUT);
	s3c_gpio_setpull(ak7343->sda->pin, S3C_GPIO_PULL_NONE);
	usleep_range(1, 1);
	if (fimc_is_sec_ak7343_get_ack(ak7343) < 0)
		ret = -EIO;
	s3c_gpio_cfgpin(ak7343->sda->pin, S3C_GPIO_OUTPUT);
	s3c_gpio_setpull(ak7343->sda->pin, S3C_GPIO_PULL_NONE);
	gpio_set_value(ak7343->sda->pin, 0);
...
int fimc_is_sec_ak7343_input(struct fimc_is_i2c_gpio *ak7343, unsigned char *data)
{
	...
	for (i = 7; i >= 0; i--) {
		...
		gpio_set_value(ak7343->scl->pin, 1);
		usleep_range(2, 2);
		read_data = gpio_get_value(ak7343->sda->pin);
		if (read_data)
			result = result | (0x1 << i);
		usleep_range(1, 1);
		gpio_set_value(ak7343->scl->pin, 0);
...
void fimc_is_sec_ak7343_stop_bit(struct fimc_is_i2c_gpio *ak7343)
{
	...
	gpio_set_value(ak7343->scl->pin, 1);
	usleep_range(2, 2);
	gpio_set_value(ak7343->sda->pin, 1);
	usleep_range(2, 2);
}
...
{
	int ret = 0;
	fimc_is_sec_ak7343_start_bit(ak7343);
	if (fimc_is_sec_ak7343_output(ak7343, AK7343_SLAVE_ADDR | AK7343_READ_BIT) < 0) {
		err("%s(%d) failed!\n", __func__, __LINE__);
		ret = -EIO;
	}
	if (fimc_is_sec_ak7343_output(ak7343, addr) < 0) {
		err("%s(%d) failed!\n", __func__, __LINE__);
		ret = -EIO;
	}
	if (fimc_is_sec_ak7343_input(ak7343, data) < 0) {
		err("%s(%d) failed!\n", __func__, __LINE__);
		ret = -EIO;
	}
	fimc_is_sec_ak7343_stop_bit(ak7343);
...
int fimc_is_sec_ak7343_write(struct fimc_is_i2c_gpio *ak7343, unsigned char addr, unsigned char data)
{
	int ret = 0;
	fimc_is_sec_ak7343_start_bit(ak7343);
	if (fimc_is_sec_ak7343_output(ak7343, AK7343_SLAVE_ADDR | AK7343_WRITE_BIT) < 0) {
		err("%s(%d) failed!\n", __func__, __LINE__);
		ret = -EIO;
	}
	if (fimc_is_sec_ak7343_output(ak7343, addr) < 0) {
	...
	if (fimc_is_sec_ak7343_output(ak7343, data) < 0) {
		err("%s(%d) failed!\n", __func__, __LINE__);
		ret = -EIO;
	}
	fimc_is_sec_ak7343_stop_bit(ak7343);
...
{
	struct fimc_is_i2c_gpio ak7343;
	unsigned char data = 0;
	unsigned char ret = 0;
	struct exynos_sensor_pin *cam_io;
	struct exynos_sensor_pin *gpio;
	int i;
	bool regulator_enable;
/*
	ak7343.scl = &pdata->sensor_info[0]->sensor_gpio.cfg[3];
	ak7343.sda = &pdata->sensor_info[0]->sensor_gpio.cfg[4];
	cam_io = &pdata->sensor_info[0]->sensor_gpio.power_io;
*/
	ak7343.scl = NULL;
	ak7343.sda = NULL;
	cam_io = NULL;
	...
	for (i = 0; i < GPIO_CTRL_MAX; i++) {
		gpio = &pdata->pin_ctrls[SENSOR_SCENARIO_NORMAL][GPIO_SCENARIO_ON][i];
		if (gpio->name) {
			if (strcmp(gpio->name, "GPF0.1 (CAM_I2C0_SCL)") == 0)
				ak7343.scl = gpio;
			if (strcmp(gpio->name, "GPF0.0 (CAM_I2C0_SDA)") == 0)
				ak7343.sda = gpio;
			if (strcmp(gpio->name, "GPE0.3 (MAIN_IO_EN)") == 0)
				cam_io = gpio;
		}
	}

	if (!ak7343.scl || !ak7343.sda || !cam_io) {
		err("%s(%d) gpio get failed!\n", __func__, __LINE__);
		return -EIO;
	}
	...
	if (!is_caldata_read) {
		/* make AF 2.8v high */
		if (system_rev < 8) {
			if (fimc_is_sec_ldo_enable(dev, "main_cam_af_2v8", true) < 0) {
				err("fimc_is_sec_ldo_enable(true) failed!\n");
				goto AK7343_ERR;
			}
			regulator_enable = true;
		} else {
			s3c_gpio_cfgpin(cam_io->pin, S3C_GPIO_OUTPUT);
			gpio_set_value(cam_io->pin, 1);
		}

		usleep_range(5000, 5000);

		/* make gpio setting output  - default high*/
		s3c_gpio_cfgpin(ak7343.scl->pin, S3C_GPIO_OUTPUT);
		s3c_gpio_setpull(ak7343.scl->pin, S3C_GPIO_PULL_NONE);
		gpio_set_value(ak7343.scl->pin, 1);
		s3c_gpio_cfgpin(ak7343.sda->pin, S3C_GPIO_OUTPUT);
		s3c_gpio_setpull(ak7343.sda->pin, S3C_GPIO_PULL_NONE);
		gpio_set_value(ak7343.sda->pin, 1);

		/* enable CAM Sensor Power - same I2C bus */
		if (system_rev < 8) {
			s3c_gpio_cfgpin(cam_io->pin, S3C_GPIO_OUTPUT);
			gpio_set_value(cam_io->pin, 1);
		} else {
		...
		s5p_gpio_set_drvstr(ak7343.sda->pin, S5P_GPIO_DRVSTR_LV4);
		s5p_gpio_set_drvstr(ak7343.scl->pin, S5P_GPIO_DRVSTR_LV4);

		usleep_range(15000, 15000);

		/* Access Free */
		if (fimc_is_sec_ak7343_write(&ak7343, AK7343_ADDR, AK7343_DATA) < 0) {
			err("%s write failed!\n", __func__);
			goto AK7343_ERR;
		}
		/* read Default read parameter */
		if (fimc_is_sec_ak7343_read(&ak7343, AK7343_PID_ADDR, &data) < 0) {
			err("%s read failed!\n", __func__);
			goto AK7343_ERR;
		}
		...
		/* make AF 2.8v low */
		if (system_rev < 8) {
			if (regulator_enable && fimc_is_sec_ldo_enable(dev, "main_cam_af_2v8", false) < 0) {
				err("fimc_is_sec_ldo_enable(false) failed!\n");
				goto AK7343_ERR;
			}
		} else {
			gpio_set_value(cam_io->pin, 0);
		}

AK7343_ERR:
		/* restore gpio setting output */
		s3c_gpio_cfgpin(ak7343.scl->pin, S3C_GPIO_OUTPUT);
		s3c_gpio_setpull(ak7343.scl->pin, S3C_GPIO_PULL_NONE);
		gpio_set_value(ak7343.scl->pin, 0);
		s3c_gpio_cfgpin(ak7343.sda->pin, S3C_GPIO_OUTPUT);
		...
		/* disable CAM Sensor Power - same I2C bus */
		if (system_rev < 8) {
			gpio_set_value(cam_io->pin, 0);
		} else {
			if (regulator_enable && fimc_is_sec_ldo_enable(dev, "main_cam_af_2v8", false) < 0) {
		...
		/* set default_pid */
		ak7343_default_pid = data;
	}
	ret = ak7343_default_pid;

	printk("\nAK7343 default PID =  %d\n", ret);
	return ret;
}
...
int fimc_is_sec_fw_sel(struct device *dev, struct exynos_platform_fimc_is_sensor *pdata, char *fw_name, char *setf_name, int isSysfsRead)
{
	...
	if ((!is_caldata_read || force_caldata_dump) &&
		...
		if (cam_id == CAMERA_DUAL_FRONT) {
			if (system_rev < 8)
				ret = fimc_is_sec_gpio_enable(pdata, "GPE0.3 (MAIN_IO_EN)", true);
			else
				ret = fimc_is_sec_ldo_enable(dev, "main_cam_af_2v8", true);
			if (ret < 0) {
				err("fimc_is_sec_gpio_enable(GPE0.3 (MAIN_IO_EN), true) failed!\n");
				is_ldo_enabled = true;
				goto exit;
			}
...
exit:
	...
	if (is_ldo_enabled) {
		if (system_rev < 8)
			ret = fimc_is_sec_gpio_enable(pdata, "GPE0.3 (MAIN_IO_EN)", false);
		else
			ret = fimc_is_sec_ldo_enable(dev, "main_cam_af_2v8", false);
