...
#define MIPI_WR_TIMEOUT msecs_to_jiffies(350)
#define MIPI_RD_TIMEOUT msecs_to_jiffies(350)
	...
	u8 cmd, const u8 *buf, u32 len)
{
	...
	case MIPI_DSI_SET_MAXIMUM_RETURN_PACKET_SIZE:
		...
		s5p_mipi_dsi_clear_interrupt(dsim, INTSRC_PH_FIFO_EMPTY);

		s5p_mipi_dsi_wr_tx_header(dsim, cmd, tempbuf);

		if (!wait_for_completion_interruptible_timeout(&dsim_ph_wr_comp,
			MIPI_WR_TIMEOUT)) {
				dev_err(dsim->dev, "MIPI DSIM short packet write Timeout!\n");
				ret = -ETIMEDOUT;
				goto exit;
		}
		break;
	...
	case MIPI_DSI_DCS_READ:
		...
		if (!wait_for_completion_interruptible_timeout(&dsim_ph_wr_comp,
			MIPI_WR_TIMEOUT)) {
				dev_err(dsim->dev, "MIPI DSIM response packet write Timeout!\n");
				ret = -ETIMEDOUT;
				goto exit;
		}
		break;
	...
	case MIPI_DSI_DCS_LONG_WRITE:
	{
		...
		s5p_mipi_dsi_clear_interrupt(dsim, INTSRC_PH_FIFO_EMPTY | INTSRC_SFR_FIFO_EMPTY);
		temp = 0;
		remind = len;
		for (i = 0; i < (len/4); i++) {
			temp = i * 4;
	...
	if (enable) {
		...
		if (byte_clk_sel == DSIM_PLL_OUT_DIV8) {
			...
			if (!soc_is_exynos5250() && !soc_is_exynos3250() && !soc_is_exynos3470() && !soc_is_exynos3472()) {
				...
				s5p_mipi_dsi_set_timing_register1(dsim, 0x09, 0x30, 0x0e, 0x0a);
				s5p_mipi_dsi_set_timing_register2(dsim, 0x0c, 0x11, 0x0d);
#else
				s5p_mipi_dsi_set_b_dphyctrl(dsim, 0x190);
				...
				s5p_mipi_dsi_set_timing_register1(dsim, 0x04, 0x15, 0x0A, 0x05);
#else
...
static void s5p_mipi_dsi_d_phy_onoff(struct platform_device *pdev, bool on)
{
	struct mipi_dsim_device *dsim = platform_get_drvdata(pdev);

	if (dsim->pd->init_d_phy)
		dsim->pd->init_d_phy(pdev->id, on);
}
...
static int s5p_mipi_dsi_init_dsim(struct mipi_dsim_device *dsim)
{
	...
	s5p_mipi_dsi_d_phy_onoff(pdev, 1);
...
static int s5p_mipi_dsi_enable(struct mipi_dsim_device *dsim)
{
	...
	/* enable interrupts */
	s5p_mipi_dsi_set_interrupt_mask(dsim, SFR_PH_FIFO_EMPTY | SFR_PL_FIFO_EMPTY | RX_DAT_DONE, 0);
...
static int s5p_mipi_dsi_disable(struct mipi_dsim_device *dsim)
{
	...
	s5p_mipi_dsi_d_phy_onoff(pdev, 0);
