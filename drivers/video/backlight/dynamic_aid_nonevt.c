...
static int calc_output_voltages(struct dynamic_aid_info d_aid)
{
	...
	for (; iv > 0; iv--) {
		...
		if(v_idx == 35) {
			divide = 70;
			for (i = v_cnt-1; i >= 0; i--, v_idx--) {
				for (c = 0; c < CI_MAX; c++)
					output_voltages[v_idx].rgb[c] = v.rgb[c] + v_diff.rgb[c]*d_aid.param.volt_table_v35[i]/divide;
				aid_dbg("iv 35: output_voltages[%d] = %lld,%lld,%lld, volt = %d\n", v_idx, output_voltages[v_idx].rgb[0], output_voltages[v_idx].rgb[1], output_voltages[v_idx].rgb[2], d_aid.param.volt_table_v35[i]);
			}
		}else if(v_idx == 15) {
			divide = 52;
			for (i = v_cnt-1; i >= 0; i--, v_idx--) {
				for (c = 0; c < CI_MAX; c++)
					output_voltages[v_idx].rgb[c] = v.rgb[c] + v_diff.rgb[c]*d_aid.param.volt_table_v15[i]/divide;
				aid_dbg("iv 15: output_voltages[%d] = %lld,%lld,%lld, volt = %d\n", v_idx, output_voltages[v_idx].rgb[0], output_voltages[v_idx].rgb[1], output_voltages[v_idx].rgb[2],d_aid.param.volt_table_v15[i]);
			}
		} else {
	...
	for (c = 0; c < CI_MAX; c++) {
		...
		output_voltages[1].rgb[c] = point_voltages[0].rgb[c];
	}
...
int dynamic_aid(struct dynamic_aid_param_t param, int **gamma)
{
	...
	d_aid.ibr_top = param.ibr_tbl[param.ibr_max-1]; /* number of top brightness */
