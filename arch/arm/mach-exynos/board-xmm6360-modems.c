...
#endif
#ifdef CONFIG_MACH_M2ALTE_USA_ATT
	{
		.name = "umts_ciq",
...
#if defined(GPIO_UART_RXD_CP) && defined(GPIO_UART_TXD_CP)
extern struct class *sec_class;
static ssize_t show_uart_cp(struct device *dev,
		struct device_attribute *attr, char *arg)
{
	return 1;
}

static ssize_t store_uart_cp(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int ret;

	if (*buf == '1') {
		ret = s3c_gpio_cfgpin(GPIO_UART_RXD_CP, S3C_GPIO_SFN(0x2));
		if (ret < 0) {
			mif_err("Fail to change gpio config UART_RXD\n");
			return ret;
		}
		ret = s3c_gpio_cfgpin(GPIO_UART_TXD_CP, S3C_GPIO_SFN(0x2));
		if (ret < 0) {
			mif_err("Fail to change gpio config UART_TXD\n");
			return ret;
		}
		mif_info("GPIO set to UART func\n");
	} else if (*buf == '0') {
		ret = gpio_direction_output(GPIO_UART_RXD_CP, 0);
		if (ret < 0) {
			mif_err("Fail to set direction_output UART_RXD\n");
			return ret;
		}
		ret = gpio_direction_output(GPIO_UART_TXD_CP, 0);
		if (ret < 0) {
			mif_err("Fail to set direction_output UART_TXD\n");
			return ret;
		}
		mif_info("GPIO set to output0\n");
	}

	return count;
}

static struct device_attribute attr_uart_cp =
	__ATTR(control, S_IRUGO | S_IWUSR, show_uart_cp, store_uart_cp);
...
static void board_variant_settings(void)
{
	 ...
	 */
	if (umts_modem_data.gpio_pda_active < EXYNOS5260_GPX0(0)
		|| umts_modem_data.gpio_pda_active > EXYNOS5260_GPX3(7))
		umts_modem_data.pda_active_hwctl = 1;
#endif

#if defined(GPIO_UART_RXD_CP) && defined(GPIO_UART_TXD_CP)
	{
		struct device *uart_cp;
		int ret;

		BUG_ON(!sec_class);
		uart_cp = device_create(sec_class, NULL, 0, NULL, "uart_cp");
		BUG_ON(!uart_cp);

		ret = device_create_file(uart_cp, &attr_uart_cp);
		if (ret) {
			mif_err("failed to create `uart_cp'\n");
			return;
		}

		ret = gpio_request(GPIO_UART_RXD_CP, "UART_RXD_CP");
		if (ret) {
			mif_err("fail to request gpio %s : %d\n", "UART_RXD_CP", ret);
			return;
		}

		ret = gpio_request(GPIO_UART_TXD_CP, "UART_TXD_CP");
		if (ret) {
			mif_err("fail to request gpio %s : %d\n", "UART_TXD_CP", ret);
			return;
		}
...
static int __init init_modem(void)
{
	...
	if(system_rev <= 0x3) { // EMUL Board
		umts_modem_data.gpio_pda_active = EMUL_GPIO_PDA_ACTIVE;
	}
#endif
